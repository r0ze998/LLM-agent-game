...
export function updateTitleScene(scene: TitleScene, deltaMs: number): void {
  ...
  // Building emerge (over ~10 seconds)
  const emergeRate = 0.0001; // per ms
...
export function renderTitleScene(
  ...
  const pulseAlpha = 0.12 + 0.04 * Math.sin(scene.time * 0.0008);
  ...
  for (const s of scene.silhouettes) {
    if (s.layer === 0) {
      drawBezierHill(ctx, s.x, s.width, s.height, groundY - h * 0.05, w, h, s.color);
    }
  }
  ...
  for (const s of scene.silhouettes) {
    if (s.layer === 1) {
      drawBezierHill(ctx, s.x, s.width, s.height, groundY, w, h, s.color);
    }
  }

  // 6. Agent connection lines
  const agents = scene.particles.filter(p => p.type === 'agent');
  ctx.strokeStyle = 'rgba(90, 138, 255, 0.06)';
  ctx.lineWidth = 0.5;
  ...
  // 7. Foreground silhouettes (buildings/trees/towers — layer 2)
  for (const s of scene.silhouettes) {
